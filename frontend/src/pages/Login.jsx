...
if (token) {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      {currentState === 'Login' ? '' : <input onChange={(e)=>setName(e.target.value)} value={name} required placeholder='Name' type="text" className='w-full px-3 py-2 border border-gray-800' />}
      ...
      <div className='w-full flex justify-between text-sm mt-[-8px]'>
        <p className='cursor-pointer text-blue-800'>Forgot your password?</p>
        {
          currentState === 'Login'
            ?
            <b onClick={() => setCurrentState('Sign Up')} className='cursor-pointer text-red-800'>Create account</b>
            : <b onClick={() => setCurrentState('Login')} className='cursor-pointer text-red-800'>Login Here</b>
        }
      </div>
